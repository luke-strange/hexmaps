...
(function(root){
	...
	function HexMap(el,attr){
		...
		var svg,hexes,lines,overlay,datalayer;
		var range = {};
		...
		this.toFront = function(region){
			var outline = this.areas[region].hex.cloneNode(true);
				...
				for(q = range.q.min; q <= range.q.max; q++){
					for(r = range.r.min; r <= range.r.max; r++){
						...
						if(this.options.showgrid){
							hex = svgEl('path');
							setAttr(hex,{'d':h.path,'class':'hex-grid','data-q':(q||"0"),'data-r':(r||"0"),'fill':(this.style.grid.fill||''),'fill-opacity':(typeof this.style.grid['fill-opacity']==="number" ? this.style.grid['fill-opacity'] : 0.1),'stroke':(typeof this.style.grid.stroke==="string" ? this.style.grid.stroke : '#aaa'),'stroke-opacity':(typeof this.style.grid['stroke-opacity']==="number" ? this.style.grid['stroke-opacity'] : 0.2)});
							add(hex,this.grid);
				...
				for(r in this.mapping.hexes){
					if(this.mapping.hexes[r]){
						...
						if(!constructed){
							...
							setAttr(g,{'data-id':r,'role':'cell'});
							hexes.appendChild(g);
							path = svgEl('path');
							setAttr(path,{'d':h.path,'transform-origin':x+'px '+y+'px'});
							setAttr(g,{'data-q':this.mapping.hexes[r].q,'data-r':this.mapping.hexes[r].r});
							...
							if(this.options.showlabel){
								if(this.style['default']['font-size'] >= this.options.minFontSize){
									...
									// Add to DOM
									g.appendChild(label);
									setAttr(label,{'x':x,'y':y,'transform-origin':x+'px '+y+'px','dominant-baseline':'central','data-q':this.mapping.hexes[r].q,'data-r':this.mapping.hexes[r].r,'class':'hex-label','text-anchor':'middle','font-size':this.properties.s.sin+'px','title':(this.mapping.hexes[r].n || r),'_region':r});
									if(this.options.clip) setAttr(label,{'clip-path':'url(#'+this.areas[r].clipid+')'});
									this.areas[r].label = label;
									this.areas[r].tooltip = tt;
